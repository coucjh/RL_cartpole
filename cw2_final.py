...
def optimize_model(memory, BATCH_SIZE, state_dim, policy_net, target_net, GAMMA, optimizer, DDQL, target_net_on):
    ...
    non_final_mask = torch.tensor(tuple(map(lambda s: torch.sum(s[0][len(s[0]) - 4:]).absolute().item() > 0,
    ...
    if sum(non_final_mask) > 0:
        non_final_next_states = torch.cat([s for s in batch.next_state
                                                    if torch.sum(s[0][len(s[0]) - 4:]).absolute().item() > 0 ])
    else:
    ...
    with torch.no_grad():
        ...
        if sum(non_final_mask) > 0:
            if not DDQL:
                ...
                # DDQL case, select action from policy network and then values from target nwtork
                policy_actions = \
...
def train(NUM_EPISODES = 100,
    ...
    durations = []  # Empty list to append total rewards for each episode to (same as duration of episode)
    epsilon = EPS_START
    eps_schedule = eps_decay(NUM_EPISODES, EPS_START, EPS_END, EPS_DECAY) # Use custom function to get decay schedule
    ...
    for i_episode in range(NUM_EPISODES):
        ...
        for t in count():
            ...
            flattened_k_states = torch.stack(list(k_states)).reshape(-1).unsqueeze(0) # Flatten the stacked frames
            action = select_action(flattened_k_states, epsilon, n_actions, policy_net) # Select action from network
            ...
            if not done:
                ...
                next_state = torch.zeros_like(state) # If terminal set next state as zeros

            # Store the transition in memory
            k_states.append(next_state)  # Append state to frame stacker deque, pushing out oldest frame
            next_flattened_k_states = torch.stack(list(k_states)).reshape(-1).unsqueeze(0) # Flatten k states again
            memory.push(flattened_k_states, action, next_flattened_k_states, reward) # Push (s,a,s',r) to memory

            # Move to the next state
            state = next_state

            # Perform one step of the optimization (on the policy network)
            optimize_model(memory, BATCH_SIZE, state_dim, policy_net, target_net, GAMMA, optimizer, DDQL, target_net_on)
            if done:
                break
        ...
        if i_episode % network_sync_freq == 0: # Sync target network with policy net every set number of episodes.
            target_net.load_state_dict(policy_net.state_dict())
...
def record_cart(policy_net, k):
    '''
    Record function separated to be toggled in main training loop, if you want to see the video.
    '''
    env = gym.make("CartPole-v1")
    ...
    while not done:
        ...
        flattened_k_states = torch.stack(list(k_states)).reshape(-1).unsqueeze(0)
        action = select_action(flattened_k_states, current_eps=0, n_actions=2, policy_net=policy_net)
...
def plot_learning_rate_full(total_rewards):
    ...
    line with errorbars.
    Total rewards given as a numpy array (R x N) with each row as a full training run
...
def plot_k_learning_rates(k_total_rewards):
    ...
    k_total_rewards given as a numpy array (R x N) with each row as a full training run
    under certain conditions:
    R = number of runs
    N = number of episodes per run
...
def plot_replay_deviation(replays_experiment, total_rewards):
    '''
    Function to plot standard deviation against size of replay buffer using a log
    x axis.
    total_rewards: given as a numpy array (R x N) with each row as a full training run
    under certain conditions
    R = number of runs
    N = number of episodes per run
...
def run_learning_curve(NUM_EPISODES, N_RUNS, k, replays, target, DDQL):
    '''
    Train agent over specified number of N_RUNS and plot the averaged learning curve
    return total_rewards as an an (R x N) matrix:
    R = number of runs
    N = number of episodes per run
    '''
...
def run_replays_experiment(NUM_EPISODES, replays_experiment):
    '''
    Train agent over different replay buffer sizes as a list (replay_experiment), and
    plot results.
    '''
    total_rewards = np.zeros(NUM_EPISODES)
...
def run_k_experiment(NUM_EPISODES, N_RUNS, k_experiment):
    '''
    Train agent over different k values (number of frames to stack) as a list (k_experiment),
    run each for N_RUNS and plot mean results.
    '''
    k_total_rewards = []
    total_rewards = np.zeros(NUM_EPISODES)
    ...
    for run in range(len(k_experiment)):
        ...
        total_rewards = run_learning_curve(NUM_EPISODES, N_RUNS, k, replays=10000, target=True, DDQL=True)
...
def run_ab_experiment(NUM_EPISODES, N_RUNS):
    '''
    Train agent ablating and augmenting different features. Run each for N_RUNS and
    ...
    for run in range(4):
        if run == 0:
            print("Normal run")
            total_rewards = run_learning_curve(NUM_EPISODES, N_RUNS, k=1, replays=10000, target=True, DDQL=False)
        elif run == 1:
            print("Ablating target network feature")
            total_rewards = run_learning_curve(NUM_EPISODES, N_RUNS, k=1, replays=10000, target=False, DDQL=False)
        elif run == 2:
            print("Ablating replay buffer")
            total_rewards = run_learning_curve(NUM_EPISODES, N_RUNS, k=1, replays=1, target=True, DDQL=False)
        elif run == 3:
            print("Implementing DDQN")
            total_rewards = run_learning_curve(NUM_EPISODES, N_RUNS, k=1, replays=10000, target=True, DDQL=True)
...
# Run various 'experiments' in main run code below. Uncomment experiments that are needed.

# Run learning curve for chosen parameters
run_learning_curve(NUM_EPISODES=200, N_RUNS=10, k=1, replays=10000, target=True, DDQL=False)

# Run replays experiment for chosen parameters
# run_replays_experiment(NUM_EPISODES=200, replays_experiment = [1, 10, 100, 1000, 10000, 100000])
